...
const ShortAnswer = (props) => {
  ...
  return (
    <div className="flex flex-col gap-5 px-3 py-2 bg-slate-800 w-250">
      ...
      {props.selected == "" ? (
        <button
          disabled={loading}
          onClick={handleShort}
          className="px-3 py-2 rounded-lg bg-slate-700 hover:bg-slate-600 transition-all duration-300 cursor-pointer flex items-center justify-center"
        >
          {loading ? <Spinner className="w-8 h-8" /> : "Submit Answer"}
        </button>
      ) : (
        <div className="px-3 py-2 rounded-lg bg-slate-700 hover:bg-slate-600 transition-all duration-300 flex items-center justify-center text-lg">
