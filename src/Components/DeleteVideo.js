  ...
  return (
    <Dialog open={open} onOpenChange={setopen}>
      <DialogTrigger
        render={
          <Tooltip>
            <TooltipTrigger onClick={() => setopen(true)} className={`p-2 rounded-full ${toggleTheme ? "bg-slate-200 hover:bg-slate-100":"bg-slate-700 hover:bg-slate-600"}  cursor-pointer transition-all duration-300`}>
              <MdDelete size={20} />
            </TooltipTrigger>
      ...
      <DialogContent className={`${toggleTheme ? "bg-slate-100 text-slate-800" : "bg-slate-800 text-gray-100"} w-100`}>
        ...
        <button
          ...
          className="bg-red-600 hover:bg-red-500 transition-all duration-300 w-full py-2 text-lg select-none rounded-xl cursor-pointer flex items-center justify-center text-white"
        >
