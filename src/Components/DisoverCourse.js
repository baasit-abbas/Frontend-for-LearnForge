...
const DisoverCourse = (props) => {
  ...
  return (
    <div className={`card flex flex-col justify-center gap-4 rounded-md ${toggleTheme ? "bg-slate-300":"bg-slate-800"} md:w-140 w-full p-2`}>
      ...
      <p className={`w-full py-2 text-center rounded-md ${toggleTheme ? "bg-slate-200":"bg-slate-700 "}`}>
        {props.description}
      </p>
      <div className="flex items-center justify-between py-2 w-full">
        <div className="flex flex-col justify-center gap-1 font-bold px-4 py-1">
          <h1 className={`${toggleTheme ? "text-slate-900":"text-slate-400"}text-slate-400`}>Author</h1>
          <p>{props.instructor}</p>
        </div>
        <button
          onClick={() => handleEnroll(props.id)}
          disabled={loading}
          className={`cursor-pointer px-3 py-2 rounded-md ${toggleTheme ? "bg-slate-100 hover:bg-slate-50":"bg-slate-950 hover:bg-slate-900"}  transition-all duration-300 flex items-center justify-center`} 
        >
          {loading ? (
